...
const SignIn: React.FC = () => {
  ...
  const handleLogin = () => {
    ...
    if (username == '' || password == '') {
      toast.warn("Please input username and password");
      return;
    }

    axios.post(`${config.SERVER_URL}user/login`, requestData).then(res => {
      if (res.data.status == 'success') {
        localStorage.setItem("token", res.data.token);
        localStorage.setItem("user", JSON.stringify(res.data.user));

        axios.defaults.headers.common['Authorization'] = res.data.token;
        navigate("/make-course");
      }
      if (res.data.status == 'not_exist') {
        toast.warn("This user isn't exist. Please register now");
      }
      if (res.data.status == 'success') {
        ...
        <div className="flex flex-wrap items-center">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <span className="mb-1.5 block font-medium">Start for Quiz</span>
